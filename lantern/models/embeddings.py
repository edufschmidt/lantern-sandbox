...
class HarmonicEmbedding(torch.nn.Module):
    def __init__(
        ...
        in embedding[...]:
            ...
            ```
        where N corresponds to `num_harmonic_functions`, and f_i is a scalar
        ...
        powers of 2:
            `f_1 = 1, ..., f_N = 2**torch.arange(num_harmonic_functions)`

        If `logspace==False`, frequencies are linearly spaced between
        `1.0` and `2**(num_harmonic_functions-1)`:
            `f_1, ..., f_N = torch.linspace(
                1.0, 2**(num_harmonic_functions-1), num_harmonic_functions
            )`
        ...
        if logspace:
            frequencies = 2.0 ** torch.arange(
                num_harmonic_functions,
                dtype=torch.float32,
            )
        else:
            frequencies = torch.linspace(
                1.0,
                2.0 ** (num_harmonic_functions - 1),
                num_harmonic_functions,
                dtype=torch.float32,
            )
    ...
    def forward(self, x: torch.Tensor):
        ...
        Returns:
            embedding: a harmonic embedding of `x` of shape
                [..., dim * (num_harmonic_functions * 2 + T)] where
